...
- **Step 3 — 差分隱私**：client 上傳前先把 weight update **裁剪**到 L2 範數 `C`、再加高斯噪音（**DP-FedAvg**，噪音 std `= z·C`），並用 **RDP** 把 noise multiplier `z` 換算成隱私預算 **ε**。掃描 `z`，量測準確率（效用）與 DLG 還原品質（隱私）隨 ε 的變化。
...
非 IID 切分（每個 client 只持有不重疊的 10 個 subject）下，**本地 epoch 越多、client drift 越大、IID 與 non-IID 的差距越開**：E=1 時兩者相同，到 E=20 時 IID 升到 0.95、non-IID 停在 0.90（差距 ~5pp）。本專案只有 4 client、全參與，drift 有限，故差距溫和；真正劇烈的 non-IID 崩潰要在 client 數很多時才會出現。
...
| 攻擊成功率隨訓練進度崩潰（16 名受害者中 PSNR > 20 dB 的比例） | iDLG vs DLG 的收斂速度 |
|:---:|:---:|
...
| 階段 | 結果 |
|------|------|
| FL 收斂（3 seed） | FedAvg 0.88±0.01、centralized 0.90±0.02（相當）|
| FL non-IID（client drift） | 本地 epoch `1→20`：E=1 IID/non-IID 同為 0.888，E=20 時 IID 0.946 vs non-IID 0.896（差距 0→~5pp，4 client 全參與下 drift 溫和）|
| DLG（未訓練模型） | 8/8 完美還原，平均 83.8 dB |
| DLG vs batch size | batch `1 → 8`：80 → 18 dB |
| DLG vs 訓練進度 | 16 名受害者攻擊成功率（PSNR>20 dB）：round 1–10 維持 ~100%，round 12–20 由 69% 緩降到 50%，round 25 掉到 6%、round 30 起 0%（隱私臨界落在 round 20–25）|
| DP 防禦（DP-FedAvg，clip C=7 + 高斯噪音，ε 由 RDP 計） | 隱私軸是連續曲線：`z=0→0.001→0.002→0.003` 把 DLG 從 84→22→12→8 dB（knee 落在此），`z=0.002` 即跌破 20 dB 門檻擋住攻擊、準確率仍 0.90；但有意義的 ε（≲60，需 z=1）對應的準確率已掉到隨機——高維小模型下 DP 代價極高，與 HE 形成對比 |
