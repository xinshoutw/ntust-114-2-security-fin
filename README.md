...
- **Step 1 — 聯邦學習**：手刻 FedAvg，4 個 client、IID 切分，在 ORL 人臉上訓練一個 LeNet（DLG 論文版：Sigmoid + strided conv，約 38K 參數）。用三條 baseline 把聯邦的價值夾出來：**centralized 0.91（資料集中、上界）≥ FedAvg 0.89（資料留本地、聚合更新）≫ local-only 0.53（各 client 只用自己 1/4 資料、不聚合、下界）**——聚合補回約 36 pp，這就是「為什麼要聯邦」。
...
準確率與損失皆為 **3 個 seed 的 mean ± std**（單 seed 曲線會被 test-set 雜訊蓋過）。三條線把聯邦的價值夾出來——**centralized ≥ FedAvg ≫ local-only**：FedAvg 幾乎追平 centralized（資料不出本地卻拿到集中訓練的準確率），而完全不聚合的 local-only 卡在 ~0.53：

| 準確率（centralized ≥ FedAvg ≫ local-only） | 損失 |
|:---:|:---:|
...
| 準確率優雅下降 vs ε（DLG 洩漏：z=0 乾淨梯度 84 dB，一加噪即崩到 ~5 dB） | 不同 `z`（對應 `ε`）下的還原 |
|:---:|:---:|
...
| 階段 | 結果 |
|------|------|
| FL 收斂（3 seed） | centralized 0.91±0.03 ≥ FedAvg 0.89±0.03 ≫ **local-only（不聚合）0.53±0.01**——聚合補回約 36 pp，量化聯邦的價值 |
...
| **DP-SGD（record-level，逐樣本裁剪 + 子取樣 RDP）** | **準確率優雅下降**：ε 2824/1046/296/156 ↔ acc 0.94/0.89/0.76/0.56，ε≈98 才崩到隨機。比 update-level 漂亮得多（同 ε 下準確率高一個檔次），但 ε<10 仍不可得；子取樣（q=0.25）把 ε 壓到 49 卻也把準確率打到隨機（維度詛咒）。DLG 洩漏 z=0 乾淨梯度 84 dB、一加噪即 ~5 dB（與 DP-FedAvg 同：經驗隱私便宜，兩機制差別在準確率軸而非洩漏軸）|
