...
| 攻擊成功率隨訓練進度崩潰（PSNR > 20 dB 的受害者比例） | iDLG vs DLG 的收斂速度 |
|:---:|:---:|
...
| DLG vs 訓練進度 | 8 名受害者攻擊成功率（PSNR>20 dB）：round 1–10 維持 7–8/8，round 12–20 掉到 5/8，round 25 起 0/8（隱私臨界落在 round 20–25）|
| DP 防禦（DP-FedAvg，clip C=7 + 高斯噪音，ε 由 RDP 計） | 經驗上 `z=0.01`（ε≈3e5）就把 DLG 84→6 dB 而準確率仍 0.89；但要拿到有意義的 ε（≲60，需 z=1）時準確率已掉到隨機水準——高維小模型下 DP 的代價極高，與 HE 形成對比 |
