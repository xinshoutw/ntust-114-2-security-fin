...
- FL: 4 clients, FedAvg (sample-weighted), IID split, 50 rounds, Adam local optimizer. Convergence is reported as a 3-way bracket — **centralized ~0.91 ≥ FedAvg ~0.89 ≫ local-only (no aggregation, `train_local_only`) ~0.53** — so the FedAvg-vs-local-only gap (~36 pp) quantifies the *value* of federation, not just "FedAvg ≈ centralized". Also supports `split="dirichlet"` (tunable α label skew) and partial participation (`client_sample_rate`) for the non-IID drift experiment, which uses 10 clients @ 50% participation: IID ~0.91 vs Dirichlet α=0.1 ~0.80 with 3× the variance (the old 4-client/full-participation block split barely moved, so it was redesigned)
