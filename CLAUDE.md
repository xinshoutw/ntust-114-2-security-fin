...
- DLG attack: LBFGS optimizer, 300 iterations; iDLG label inference; attacks the single-sample loss gradient (not the multi-step FedAvg delta)
- Step 3 DP: DP-FedAvg — per-client L2 clip + Gaussian noise (std = z·C); privacy budget ε via RDP composition
